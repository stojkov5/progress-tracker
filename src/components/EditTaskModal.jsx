...
const EditTaskModal = ({ isVisible, handleClose, task, refetch }) => {
  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      ...
      const updatedTasks = storedTasks.map((t) => (t.id === task.id ? { ...t, ...values } : t));
  ...
  return (
    <Modal title="Edit Task" open={isVisible} onCancel={handleClose} footer={null}>
      <Form layout="vertical" onFinish={formik.handleSubmit}>
        <Form.Item label="Title" help={formik.touched.title && formik.errors.title}>
          <Input name="title" onChange={formik.handleChange} value={formik.values.title} ref={titleInputRef} />
        </Form.Item>
